...
public:
    ...
    /***************************************************************************/
    //public
    void insertar(int k, int f, vector<string> frms, Mat img, Mat croppedImg) {
        root = insertarAVL(root, k, f, frms, img, croppedImg);
    }
    //private
    NodoAVL* insertarAVL(NodoAVL* nodoActual, int k, int f, vector<string> frms, Mat img, Mat croppedImg) {
        if (nodoActual == nullptr) {
            //cout << "Nulo el nodo" << endl;
            return (new NodoAVL(k, f, frms, img, croppedImg));
        }

        if (f < nodoActual->frec) {
            nodoActual->left = insertarAVL(nodoActual->left, k, f, frms, img, croppedImg);
        }
        else if (f > nodoActual->frec) {
            nodoActual->right = insertarAVL(nodoActual->right, k, f, frms, img, croppedImg);
        }
